...
func _process(_delta: float) -> void:
	if player_in_range and Input.is_action_just_pressed("ui_accept") and not Inventaire.is_inventory_active:
		if StoryManager.is_playing:
			...
			if quest.step >= 1 and quest.step < 3:
				emit_signal("start_new_dialogue", 11)
				if quest.step == 1:
					quest.advance_step()
			elif quest.step == 3 or quest.step < 1: # Le pnj ne connait pas encore la quête ou la quête est finie -> comportement normal
				emit_signal("start_new_dialogue", 13)
